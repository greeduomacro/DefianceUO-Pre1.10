		...
		public HouseGumpAOS( HouseGumpPageAOS page, Mobile from, BaseHouse house ) : base( 50, 40 )
		{
			...
			switch ( page )
			{
				...
				case HouseGumpPageAOS.Security:
				{
					...
					AddButtonLabeled( 210, 180, GetButtonID( 3, 14 ), 1011247, isOwner ); // Clear Friend List
				...
				case 3:
				{
					switch ( index )
					{
						...
						case 12: // Make Private
						{
							...
							foreach (Item item in from.GetItemsInRange(20))
								if (item is TownHouse)
									thlist.Add(item);

							foreach (TownHouse t in thlist)
								if (t.Owner == from)
								{
									th = t;
									break;
								...
								{
									from.SendMessage("This house cannot be private.");
									break;
								}
						...
						case 13: // Make Public
						{
							if ( isOwner )
							{
								...
								if ( th != null && th.ForSaleSign != null && th.ForSaleSign.ForcePrivate )
								{
									from.SendMessage("This house cannot be public.");
									break;
								}
					...
					switch ( index )
					{
						...
						case 1: // Trade House
						{
							if ( isOwner )
							{
								...
								if ( th != null && th.ForSaleSign != null && th.ForSaleSign.NoTrade )
								{
									from.SendMessage("This house cannot be traded.");
								}
								else if ( BaseHouse.NewVendorSystem && m_House.HasPersonalVendors )
								{
