...
namespace Server.Gumps
{
	...
	public class ResurrectGump : Gump
	{
		...
		private void CloseOldGumps( Mobile owner )
		{
			...
			if ( ns != null )
			{
				...
				{
					if ( ns.Gumps[i] is ResurrectGump && ns.Gumps[i] != null && ns.Gumps[i] != this )
						ns.Gumps.RemoveAt(i);
				}
			}
		}
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( info.ButtonID == 1 || info.ButtonID == 2 )
			{
				...
				from.Resurrect();
                                //add by xlx to avoid instant fattigue when ressing.
                                from.Hits = 25;
                                //add by xlx end.
