...
namespace Server.Gumps
{
	public class ReportMurdererGump : Gump
	{
		...
		private Map m_Map;
		private const int m_MaxBounty = 500000000; // 500 million :D
		//private int const m_MaxBounty = 5000000000;
		...
		public static void EventSink_PlayerDeath( PlayerDeathEventArgs e )
		{
			...
			{
                //Al: Find out if we are in a Custom Region which has disabled murder counts
                bool isNoMurderCountsCustomRegion = ((m.Region is CustomRegion) && (((CustomRegion)m.Region).NoMurderCounts));
                if (ai.Attacker.Player && ai.CanReportMurder && !ai.Reported && !isNoMurderCountsCustomRegion)
				{
					...
					foreach (PlayerMobile.Report report in ((PlayerMobile)ai.Attacker).Reports)
					{
						if (DateTime.Now - report.ReportTime < TimeSpan.FromMinutes(5.0))
						{
							filteredReports.Add(report);
							if (report.Reporter == m)
							{
								canReport = false;
							}
						}
					}
					((PlayerMobile)ai.Attacker).Reports = filteredReports;
					...
					//These two lines must be commented back in on removal of MURDERREPORT edit^^
					//killers.Add( ai.Attacker );
					//ai.Reported = true;
				}
		...
		private class GumpTimer : Timer
		{
			...
			public GumpTimer( Mobile victim, ArrayList killers, Point3D loc, Map map ) : base( TimeSpan.FromSeconds( 4.0 ) )
			{
				m_Victim = victim;
				m_Killers = killers;
				m_Location = loc;
		...
		public ReportMurdererGump( Mobile victum, ArrayList killers, Point3D loc, Map map ) : this( victum, killers, loc, map, 0 )
		{
		}

		private ReportMurdererGump( Mobile victum, ArrayList killers, Point3D loc, Map map, int idx ) : base( 0, 0 )
		{
			m_Killers = killers;
			m_Victum = victum;
			m_Location = loc;
		...
		private void BuildGump()
		{
			...
			AddTextEntry( 260, 350, 100, 15, 0, 0, "0" );
			AddHtml( 260, 330, 300, 50, String.Format( "Optional Bounty ({0} max)",  m_Victum == null ? 0 : Banker.GetBalance( m_Victum ) ), false, false );
			...
			switch ( info.ButtonID )
			{
				case 1:
				{
					...
					if ( killer != null && !killer.Deleted )
					{
						...
						//if( ( region is GuardedRegion ) || ( region is DungeonRegion || (gregion != null && !gregion.IsDisabled())) || ( cregion != null && ( cregion.Controller.IsDungeonRegion || cregion.Controller.IsGuardedRegion ) ) )
						...
						//*********ADDED MURDERREPORT 5 MIN TIMEOUT*************//
						((PlayerMobile)killer).Reports.Add(new PlayerMobile.Report(from, DateTime.Now));
						//********END OF MURDERREPORT 5 MIN TIMEOUT*************//
						...
						int max = Math.Min( Banker.GetBalance( from ), m_MaxBounty );

						if ( bounty > 0 && bounty <= max )
						{
							Banker.Withdraw( from, bounty );
							BountyTable.Add( killer, bounty );
							//from.SendMessage( "Bonty on {0} gp added!!", bounty );
						}
					}
					break;
				}
