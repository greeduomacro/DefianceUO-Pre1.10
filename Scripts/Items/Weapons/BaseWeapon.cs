...
namespace Server.Items
{
	public abstract class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable
	{
		...
		// Added for Ability Charges - Light
		private WeaponChargedAbility m_ChargedAbility;
		private int m_AbilityCharges;

        public static string GetChargedAbilityName(WeaponChargedAbility ability)
        {
            switch (ability)
            {
		...
		public WeaponChargedAbility ChargedAbility
		{
		...
		public virtual void OnHit( Mobile attacker, Mobile defender )
		{
			...
			if ( (this is CarnageDagger || this is NoBlowCarnageDagger) && 0.14 > Utility.RandomDouble() )
			{
				damage *= 3;
				attacker.SendMessage("You deliver a critical strike!");
				defender.SendMessage("You suffer a critical strike!");
			}
...
*/
			...
			ChargedAbilityAttack( attacker, defender );
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			writer.Write( (int)m_ChargedAbility ); // version 8
			writer.Write( m_AbilityCharges ); // version 8
...
//				World.buggeditems.Add( this );
		...
		public static bool IsBugged( Item item )
		{
			int hue = item.Hue;
			LootType loot = item.LootType;

			return loot == LootType.Blessed && ( hue == 1325 || hue == 1310 || hue == 1645 || hue == 1109 || hue == 1109 || hue == 2211 || hue == 1254 || hue == 2125 );
		}
