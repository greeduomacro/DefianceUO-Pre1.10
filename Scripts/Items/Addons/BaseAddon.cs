...
namespace Server.Items
{
	...
	public abstract class BaseAddon : Item, IChopable, IAddon
	{
		...
		public static bool CheckHouse( Mobile from, Point3D p, Map map, int height, ArrayList list )
		{
			if (from == null)
				/* Al:
				 * '|| from.AccessLevel >= AccessLevel.GameMaster' removed
				 * because it prevented the house list 'list' from being updated.
				 * As a result, GM placed house addons were not added to the house's
				 * addon list and they were not redeedable.
				 */
				return true;

			BaseHouse house = BaseHouse.FindHouseAt( p, map, height );

			if (house == null || !house.IsOwner( from )) //GMs can place house addons outside of houses.
