...
namespace Server.Items
{
	...
	public abstract class BaseClothing : Item, IDyable, IScissorable, IFactionItem, ICraftable
	{
		...
		private static readonly Regex ms_RegexLayeredTag = new Regex(@"(?<taglessname>.+)\(layered\)$");
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			if (Layer == Layer.Earrings || Layer == Layer.Neck) LabelTo(from, "[layered]");
		}
		...
		public bool Scissor( Mobile from, Scissors scissors )
		{
			...
			if ( item != null && item.Ressources.Count == 1 && item.Ressources.GetAt( 0 ).Amount >= 2 )
			{
				try
				{
					...
					{
						from.SendMessage("Due to the corrosive dye used to hue this item it only yields little material.");
						res.Amount = 1;
					}
