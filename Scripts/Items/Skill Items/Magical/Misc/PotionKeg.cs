	...
	public class PotionKeg : Item
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( from.InRange( GetWorldLocation(), 2 ) )
			{
                if (m_Held > 0)
                {
                    // Touched by Dev Minkio to allow to fill potions into subpacks.
                    Container pack = from.Backpack;
                    Bottle emptyBottle = null;
                    if (pack != null || pack.Deleted)
                    {
                        Item bottleItem = pack.FindItemByType(typeof(Bottle), true);
                        emptyBottle = (Bottle)bottleItem;
                        if (emptyBottle != null)
                        {
                            // pedantic check
                            if (!(emptyBottle.Parent is Container))
                                return;

                            Container potPack = emptyBottle.Parent as Container;
                            from.SendLocalizedMessage(502242); // You pour some of the keg's contents into an empty bottle...
                            BasePotion pot = FillBottle();
                            if ( pack.TotalItems < pack.MaxItems )
                            {
                                potPack.ConsumeTotal(typeof(Bottle), 1);
                                potPack.TryDropItem(from, pot, false);
                        ...
                        {
                            from.SendMessage("You dont have an empty bottle in your backpack.");
                        }
                    }
                }
                else
