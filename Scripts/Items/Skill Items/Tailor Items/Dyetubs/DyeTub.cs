...
namespace Server.Items
{
	...
	public class DyeTub : Item
	{
		...
		private class InternalTarget : Target
		{
            ...
            //al: fix for locked down furniture not being dyable "That is not accessible."
            protected override void OnTargetNotAccessible(Mobile from, object targeted)
            {
                if (m_Tub is FurnitureDyeTub)
                    //Override only for furniture dye tubs
                    OnTarget(from, targeted);
                else
                    //Keep default action for other dye tubs
						...
						else
						{
							...
							if ( !okay )
							{
								if ( item.Parent == null )
								{
									...
									//al: fix for secures not being dyable "must be locked down"
