...
namespace Server.Items
{
	public class Bola : Item
	{
		...
		private static void FinishThrow( object state )
		{
			...
			// jakob, gave them 60% to evade bola
			if ( mt is FactionWarHorse && Utility.Random( 100 ) < 60 ) // 60% to be evade bola
			{
				from.SendMessage( "Their mount evades the bola." );
				to.SendMessage( "Your mount succesfully evade the bola attempt!" );
			}
			else
			{
				if ( mt != null )
