...
namespace Server.Multis
{
	public abstract class BaseHouse : BaseMulti
	{
		...
		public const int MaxFriends = 75;
...
*/
		...
		{
			get{ DecayType type = DecayType; return ( type == DecayType.Condemned || type == DecayType.ManualRefresh ); }
		}
		...
		public virtual DecayLevel DecayLevel
		{
			get
			{
				...
				//else if ( pmowner != null && pmowner.DonationStart > DateTime.MinValue && pmowner.DonationDuration < TimeSpan.MaxValue && ( pmowner.DonationStart + pmowner.DonationDuration + DecayPeriod ) >= DateTime.Now )
		...
		public void MoveAllToCrate()
		{
			...
			foreach ( Item addon in m_Addons )
			{
				if ( !addon.Deleted )
				{
					Item deed = null;
...
*/
		...
		public bool IsOwner( Mobile m )
		{
			...
			if ( m == m_Owner || m.AccessLevel >= AccessLevel.Seer )
				return true;
