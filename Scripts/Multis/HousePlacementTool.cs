...
namespace Server.Items
{
	...
	{
		public HousePlacementTool m_Tool;
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( IsChildOf( from.Backpack ) )
				from.SendGump( new HousePlacementCategoryGump( from, this ) );
			else
	...
	public class HousePlacementCategoryGump : Gump
	{
		...
		public override void OnResponse( Server.Network.NetState sender, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
				...
				{
					m_From.SendGump( new HousePlacementListGump( m_From, HousePlacementEntry.ClassicHouses, m_Tool ) );
					break;
				}
				case 2: // 2-Story Customizable Houses
				{
					m_From.SendGump( new HousePlacementListGump( m_From, HousePlacementEntry.TwoStoryFoundations, m_Tool ) );
					break;
				}
				case 3: // 3-Story Customizable Houses
				{
					m_From.SendGump( new HousePlacementListGump( m_From, HousePlacementEntry.ThreeStoryFoundations, m_Tool ) );
					break;
				}
			}
		}
		...
		public override void OnResponse( Server.Network.NetState sender, RelayInfo info )
		{
			...
			if ( index >= 0 && index < m_Entries.Length )
			{
				if ( m_From.AccessLevel < AccessLevel.GameMaster && BaseHouse.HasAccountHouse( m_From ) )
					...
					m_From.Target = new NewHousePlacementTarget( m_Entries, m_Entries[index], m_Tool );
			}
			else
			{
				m_From.SendGump( new HousePlacementCategoryGump( m_From, m_Tool ) );
			}
		}
	}
	...
	public class NewHousePlacementTarget : MultiTarget
	{
		...
		protected override void OnTarget( Mobile from, object o )
		{
			...
			if ( ip != null )
			{
				...
				{
					m_Placed = m_Entry.OnPlacement( from, p, m_Tool );
				}
		...
		protected override void OnTargetFinish( Mobile from )
		{
			...
			if ( !m_Placed )
				from.SendGump( new HousePlacementListGump( from, m_Entries, m_Tool ) );
		}
	}
	...
	public class HousePlacementEntry
	{
		...
		private static HousePlacementEntry[] m_ClassicHouses = new HousePlacementEntry[]
			{
				new HousePlacementEntry( typeof( SmallOldHouse ),		1011303,	425,	212,	489,	244,	10,	95000,		0,	4,	0,	0x0064	),
				new HousePlacementEntry( typeof( SmallOldHouse ),		1011304,	425,	212,	489,	244,	10,	95000,		0,	4,	0,	0x0066	),
				new HousePlacementEntry( typeof( SmallOldHouse ),		1011305,	425,	212,	489,	244,	10,	95000,		0,	4,	0,	0x0068	),
				new HousePlacementEntry( typeof( SmallOldHouse ),		1011306,	425,	212,	489,	244,	10,	95000,		0,	4,	0,	0x006A	),
				new HousePlacementEntry( typeof( SmallOldHouse ),		1011307,	425,	212,	489,	244,	10,	95000,		0,	4,	0,	0x006C	),
				new HousePlacementEntry( typeof( SmallOldHouse ),		1011308,	425,	212,	489,	244,	10,	95000,		0,	4,	0,	0x006E	),
				new HousePlacementEntry( typeof( SmallShop ),			1011321,	425,	212,	489,	244,	10,	135000,	   -1,	4,	0,	0x00A0	),
				new HousePlacementEntry( typeof( SmallShop ),			1011322,	425,	212,	489,	244,	10,	135000,		0,	4,	0,	0x00A2	),
				new HousePlacementEntry( typeof( SmallTower ),			1011317,	580,	290,	667,	333,	14,	170000,		3,	4,	0,	0x0098	),
				new HousePlacementEntry( typeof( TwoStoryVilla ),		1011319,	1100,	550,	1265,	632,	24,	188000,		3,	6,	0,	0x009E	),
				new HousePlacementEntry( typeof( SandStonePatio ),		1011320,	850,	425,	1265,	632,	24,	171000,	   -1,	4,	0,	0x009C	),
				new HousePlacementEntry( typeof( LogCabin ),			1011318,	1100,	550,	1265,	632,	24,	198000,		1,	6,	0,	0x009A	),
				new HousePlacementEntry( typeof( GuildHouse ),			1011309,	1370,	685,	1576,	788,	28,	225000,	   -1,	7,	0,	0x0074	),
				new HousePlacementEntry( typeof( TwoStoryHouse ),		1011310,	1370,	685,	1576,	788,	28,	195000,	   -3,	7,	0,	0x0076	),
				new HousePlacementEntry( typeof( TwoStoryHouse ),		1011311,	1370,	685,	1576,	788,	28,	195000,	   -3,	7,	0,	0x0078	),
				new HousePlacementEntry( typeof( LargePatioHouse ),		1011315,	1370,	685,	1576,	788,	28,	255000,	   -4,	7,	0,	0x008C	),
				new HousePlacementEntry( typeof( LargeMarbleHouse ),	        1011316,	1370,	685,	1576,	788,	28,	295000,	   -4,	7,	0,	0x0096	),
				new HousePlacementEntry( typeof( Tower ),			1011312,	2119,	1059,	2437,	1218,	42,	835000,		0,	7,	0,	0x007A	),
