...
namespace Server.Multis
{
	...
	public class HouseFoundation : BaseHouse
	{
		...
		public void AddFixtures( Mobile from, MultiTileEntry[] list )
		{
			...
			for ( int i = 0; i < list.Length; ++i )
			{
				...
				else
				{
					...
					if ( door != null )
					{
						for ( int j = 0; j < Doors.Count && keyValue == 0; j++ )
						{
							keyValue = ((BaseDoor)Doors[j]).KeyValue;
							break;
						}
		...
		{
            if ( this.Deleted || DesignContext.Find( from ) == null )
				return;
...
/*
                    if (this.Deleted || DesignContext.Find(from) == null)
                    {
                        string logMsg = String.Format("From: {0}, House: {1}, House.deleted: {2}, Gold: {3}", from, this, this.Deleted, -cost);
                        GeneralLogging.WriteLine("AOSHouseCashBuguse", logMsg);
                    }
*/
                }
                else
