...
namespace Server.Mobiles
{
	...
	public abstract class BaseAI
	{
				...
				if ( e.Mobile.Alive && ( isOwner || isFriend ) )
				{
					...
					for ( int i = 0; i < keywords.Length; ++i )
					{
						...
						switch ( keyword )
						{
							...
							{
								// BEGIN
								// Part of the StamSystem (StamSystem.cs).
								// All mounts with the exeption of LechSteeds aka EvoHiryu
								// get stamina by spaming all follow me
								// by Minkio
								if (StamSystem.active && m_Mobile != null && m_Mobile is BaseMount && !(m_Mobile is EvoHiryu))
								{
									Mobile mm = (Mobile)m_Mobile as Mobile;
									double percent = (double)new Random().Next(StamSystem.followStamBonusMin, StamSystem.followStamBonusMax);
									percent /= 100;

									if (mm.Stam + (int)Math.Round(mm.StamMax * percent) > mm.StamMax)
										mm.Stam = mm.StamMax;
									else
									{
										int addToStam = (int)Math.Round(mm.StamMax * percent);
										mm.Stam += addToStam;
									}
		...
		{
			if ( m_Mobile.Deleted||m_Mobile.ControlMaster==null)//Al: Fix for NullPointerException
				return false;
...
/*Change Speed*/
					...
					if (mOnHorse)
					{
						m_Mobile.CurrentSpeed = (iCurrDist > 4 ? .15 : .20);
						WMR = WalkMobileRange(m_Mobile.ControlMaster, 1, iCurrDist > 4, 0, 1);
					}
					else
					{
						m_Mobile.CurrentSpeed = (iCurrDist > 4 ? .25 : .3);
						WMR = WalkMobileRange(m_Mobile.ControlMaster, 1, iCurrDist > 4, 0, 1);
					}
					if ( WMR )
/*Change Speed*/	{
		...
		public virtual bool DoOrderFollow()
		{
			...
			// Modified by Luthor to only follow hidden/dead targets if they are the master and in range
			else if ( m_Mobile.ControlTarget != null && (m_Mobile.ControlTarget.Alive || m_Mobile.ControlTarget == m_Mobile.ControlMaster) && (!m_Mobile.ControlTarget.Hidden || m_Mobile.ControlTarget == m_Mobile.ControlMaster) && !m_Mobile.ControlTarget.Deleted && m_Mobile.ControlTarget != m_Mobile && m_Mobile.InRange( m_Mobile.ControlTarget, m_Mobile.RangePerception * 2 ) )
			{
...
/*Change Speed*/
					...
					{
						WMR = WalkMobileRange(m_Mobile.ControlTarget, 1, false, 0, 1);
					}
					else
					{
						if (mOnHorse)
...
/*Change Speed*/	{
		...
		public virtual bool DoOrderGuard()
		{
			...
			// Changed by Luthor to only take a new target if the master is fighting it
			Mobile mastersCombatant = controlMaster.Combatant;

			if ( mastersCombatant != null )
			{
					if ( mastersCombatant != null && !mastersCombatant.Deleted && mastersCombatant.GetDistanceToSqrt( m_Mobile ) <= m_Mobile.RangePerception )
					{
						if ( combatant == null || mastersCombatant.GetDistanceToSqrt( controlMaster ) < combatant.GetDistanceToSqrt( controlMaster ) )
							combatant = mastersCombatant;
					}
...
/*Change Speed*/
				...
				{
					m_Mobile.CurrentSpeed = (iCurrDist > 4 ? .15 : .20);
					WalkMobileRange(m_Mobile.ControlMaster, 1, iCurrDist > 4, 0, 1);
				}
				else
				{
					m_Mobile.CurrentSpeed = (iCurrDist > 4 ? .25 : .3);
...
/*Change Speed*/
		...
		public virtual bool DoOrderAttack()
		{
			...
			// Modified by Luthor to include a range check
		...
		public virtual bool DoOrderRelease()
		{
			...
			m_Mobile.OwnerAbandonTime = DateTime.Now;
		...
		/*
			...
			*
			*  iRange : The range
			*  acqType : A type of acquire we want (closest, stronguest, etc
		...
		public virtual bool AcquireFocusMob(int iRange, FightMode acqType, bool bPlayerOnly, bool bFacFriend, bool bFacFoe)
		{
			...
			{
			// Modified By Luthor
				if ( m_Mobile.ControlTarget == null || m_Mobile.ControlTarget.Hidden || m_Mobile.ControlTarget.Deleted || !m_Mobile.ControlTarget.Alive || m_Mobile.ControlTarget.IsDeadBondedPet || !m_Mobile.InRange( m_Mobile.ControlTarget, m_Mobile.RangePerception * 2 ) )
				{
					m_Mobile.ControlTarget = null;
					m_Mobile.FocusMob = null;
					return false;
			...
			if ( map != null )
			{
				...
				foreach ( Mobile m in eable )
				{
					...
					if ( bCheckIt ) // alrealy passed the others tests
					{
						...
						/*if ( m is BaseCreature )
						{
							...
							else if ( bFacFriend ) // we want allies only
							{
								...
								{
									bCheckIt = ( c.Team == m_Mobile.Team && (c.Summoned || c.Controlled) );
								}
								else
								{
									bCheckIt = ( c.Team == m_Mobile.Team && !(c.Summoned || c.Controlled) );
					...
					if ( (bCheckIt && (acqType == FightMode.Agressor || acqType == FightMode.Evil)) || (!bCheckIt && m_Mobile.Summoned && !m_Mobile.Controlled) )
					{
