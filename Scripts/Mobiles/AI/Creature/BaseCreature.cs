...
namespace Server.Mobiles
{
	...
	public class BaseCreature : Mobile
	{
		...
		private static ArrayList m_CannotControl = new ArrayList();

		public static ArrayList CannotControl{ get{ return m_CannotControl; } }
...
// *** Added for Valor ***
		private bool		m_IsChampionMonster = false;
		private int			m_SpawnLevel = 0;

		public bool IsChampionMonster
		{
			get{ return m_IsChampionMonster; }
			set{ m_IsChampionMonster = value; }
		}

		public int SpawnLevel
		{
			get{ return m_SpawnLevel; }
			set{ m_SpawnLevel = value; }
		}
// *** ***
		...
		public virtual bool IsEnemy( Mobile m )
		{
            ...
            if (ourFaction != null && Faction.IsFactionMap(Map) && !RegionIsNoFactions)
			{
		...
		{
		// jakob, 0-24% damagee bonus from faction dude to untamed creatures depending on how many towns they own
			if ( !this.Controlled )
			{
				Faction faction = Faction.Find( from );
				if ( faction != null )
				{
					double final = 1.0 + DamageBonusPerOwnedTown * faction.OwnedTowns;
					damage = (int)(damage * final);
				}
			}
			// end
                }
		...
		// jakob, 0-24% damagee bonus from faction dude to untamed creatures depending on how many towns they own
			...
			// end
                ...
                // jakob, added this variable for easy bonus changing
		public const double DamageBonusPerOwnedTown = 0.03;
		// end
...
// *** ***
		...
		public Mobile ControlMaster
		{
			...
			set
			{
                ...
                //Al: Fix for crash bug when pet is in follow mode and ControlMaster is set to null
                if (m_ControlMaster == null)
                {
                    m_ControlOrder = OrderType.None;
                    m_bControlled = false;
		...
		public override void OnMovement( Mobile m, Point3D oldLocation )
		{
			...
			/* Begin notice sound */
			if ( (!m.Hidden || m.AccessLevel == AccessLevel.Player) && m.Player && m_FightMode != FightMode.Agressor && m_FightMode != FightMode.None && Combatant == null && !Controlled && !Summoned )
			{
		...
		{
            if (m_arSpellAttack == null) m_arSpellAttack = new ArrayList(4);
            m_arSpellAttack.Add ( type );
		}

		public void AddSpellDefense( Type type )
		{
			if (m_arSpellDefense == null) m_arSpellDefense = new ArrayList(4);
        ...
        private Type[] m_JewelTypes = new Type[]
        {
            typeof(GoldRing),
            typeof(GoldEarrings),
            typeof(GoldBracelet),
        ...
        public bool PackJewel(double chance)
        {
            if (chance <= Utility.RandomDouble())
                return false;

            Type type = m_JewelTypes[Utility.Random(m_JewelTypes.Length)];
            BaseJewel jewel = (BaseJewel)Activator.CreateInstance(type) as BaseJewel;
		...
		public override void OnDeath( Container c )
		{
			...
			if ( IsBonded )
			{
				...
				{
					if ( this.m_OwnerAbandonTime == DateTime.MinValue )
						this.m_OwnerAbandonTime = DateTime.Now;
				}
				else
				{
					this.m_OwnerAbandonTime = DateTime.MinValue;
				}
...
// *** ***
		...
		public virtual void OnThink()
		{
			...
			/*if (Combatant != null && Combatant.Hidden)
			{
				Combatant = null;
				ControlTarget = null;
			}*/
			...
			if ( HasBreath && !Summoned && DateTime.Now >= m_NextBreathTime ) // tested: controlled dragons do breath fire, what about summoned skeletal dragons?
			{
		...
		protected override void OnTick()
		{
			...
			foreach ( Mobile m in World.Mobiles.Values )
			{
				...
				if ( m is BaseCreature )
				{
					...
					if ( ( c is EvolutionDragon || c is IEvoCreature ) && ( c.ControlMaster == null || c.ControlMaster.Followers <= c.ControlMaster.FollowersMax ) )
					...
					else if ( c.Controlled && !c.IsStabled && c.Commandable && c.Loyalty > PetLoyalty.None && c.Map != Map.Internal )
					{
					...
					if ( !c.Controlled && !c.IsStabled && ((c.Region is HouseRegion && c.CanBeDamaged()) || (c.RemoveIfUntamed && c.Spawner == null)) )
					{
						c.RemoveStep++;
