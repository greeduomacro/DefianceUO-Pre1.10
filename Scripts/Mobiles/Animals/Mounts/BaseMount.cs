namespace Server.Mobiles
{
	public abstract class BaseMount : BaseCreature, IMount
	{
		...
		private Item m_InternalItem;
                public Item InternalItem{ get{ return m_InternalItem; } set{ m_InternalItem = value; } }
		...
		public override bool CanRegenStam
		{
			get
			{
				if ( m_Rider == null || !(m_Rider is PlayerMobile) || !Server.Misc.StamSystem.active_for_player((PlayerMobile)m_Rider) ||
					(!(this is Xanthos.Evo.EvoHiryu) && Server.Misc.StamSystem.mountregendelay == 0) ||
					(this is Xanthos.Evo.EvoHiryu && Server.Misc.StamSystem.steedregendelay == 0))
			...
			if ( from.InRange( this, 1 ) )
			{
				...
				if ( canAccess )
				{
					if ( this.Poisoned )
						...
						PrivateOverheadMessage( Network.MessageType.Regular, 0x3B2, 500205, from.NetState ); // You can't get on a mount that's fighting.
					else
						Rider = from;
				}
