			...
			get
			{
				...
				else
				{
                    ...
                    catch (OverflowException e)
                    {
                        ...
                        /* and log what happend */
                        string logMsg = String.Format("A try to create money via integer overflow was detected on playervendor {0}. The owner is {1}.", CommandLogging.Format(this), CommandLogging.Format(m_Owner));
                        GeneralLogging.WriteLine("VendorCashBuguse", logMsg);
					}
...
/*
			if ( m.AccessLevel >= AccessLevel.GameMaster )
				return true;
*/
//			return m.AccessLevel >= AccessLevel.GameMaster || ( House != null && House.IsOwner( m ) ) || m == Owner;
			return m.AccessLevel >= AccessLevel.GameMaster || m == Owner;
/*
...
*/
		...
		public virtual void Destroy( bool toBackpack, bool toOwner )
		{
			Return();
			...
			if ( list.Count > 0 || HoldGold > 0 ) // No case 1
			{
				...
				else if ( ( toBackpack || House == null || !House.IsAosRules ) && this.Map != Map.Internal ) // Case 3 - Move to backpack
				{
					...
					if ( m_Owner != null )
						m_Owner.BankBox.DropItem( backpack );
					else
		...
		private class PayTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				if ( pay > totalGold )
				{
					m_Vendor.Destroy( !BaseHouse.NewVendorSystem, false );
				}
				else
				{
					if ( !BaseHouse.NewVendorSystem )
			...
			private void GiveGold( Mobile to, int amount )
			{
				...
				else
				{
                    if (to.AccessLevel > AccessLevel.Player)
                        CommandLogging.WriteLine(to, "{0} {1} collecting {2} gold from vendor {3}", to.AccessLevel, CommandLogging.Format(to), amount, CommandLogging.Format(this));
