...
namespace Server.Mobiles
{
    ...
    public abstract class BaseVendor : BaseCreature, IVendor
    {
        ...
        private const int CashRestock = 15000; //Al: Sell limit
        ...
        private int m_cash = CashRestock;
        ...
        //Edit by Sunny
        public bool CanReceiveBulkOrder(PlayerMobile m, SkillName skill)
        {
            string message = string.Empty;

            if (m.Young) // instant logout
                message = "You cannot receive any bulk order deeds while you have the 'young' status.";

            if (m.Skills[skill].Base < 65.0)//lower than 65 = no go, you can change it
                message = "Sorry we are looking for more skilled crafters to complete our bulk orders.";//make up something yourself ;)

            if (message != string.Empty)
            {
                if (Utility.RandomDouble() < 0.33)// 1/3 chance to shout
                    Say(message);
        ...
        public virtual void Restock()
        {
            ...
            if (m_cash < CashRestock) m_cash = CashRestock;
        }
        ...
        public virtual bool OnBuyItems(Mobile buyer, ArrayList list)
        {
            ...
            if (buyer.AccessLevel >= AccessLevel.GameMaster)
                CommandLogging.WriteLine(buyer, "{0} {1} bought items from vendor {2} for {3} gold.", buyer.AccessLevel, CommandLogging.Format(buyer), CommandLogging.Format(this), totalCost);
