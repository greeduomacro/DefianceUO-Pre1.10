...
namespace Server.Misc
{
	public class FindRunes
	{
		public static void Initialize()
		{
			...
			Server.Commands.Register( "DeleteRunes", AccessLevel.Administrator, new CommandEventHandler( DeleteRunes_OnCommand ) );
		}
		...
		public static void DeleteRunes_OnCommand( CommandEventArgs e )
		{
			...
			foreach ( Item item in items )
			{
				...
				{
					Runebook book = (Runebook)item;
					ArrayList entries = book.Entries;
					for ( int i = 0; i < entries.Count; ++i )
					{
						RunebookEntry entry = (RunebookEntry)entries[i];
