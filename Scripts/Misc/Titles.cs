...
namespace Server.Misc
{
	public class Titles
	{
		...
		{
            //Al: Find out if we are in a Custom Region which has disabled fame karma
            if ((m.Region is CustomRegion) && (((CustomRegion)m.Region).NoFameKarma)) return;
					...
					if ( fame <= fe.m_Fame || i == (m_FameEntries.Length - 1) )
					{
						...
						for ( int j = 0; j < karmaEntries.Length; ++j )
						{
							...
							if ( karma <= ke.m_Karma || j == (karmaEntries.Length - 1) )
							{
								...
								{
									FSPvpSystem.PvpStats ps = FSPvpSystem.GetPvpStats( beheld );

									if ( ps.ShowPvpTitle )
									{
										string rank = PvpRankInfo.GetInfo( ps.RankType ).Title;
										title.AppendFormat( rank + " " + ke.m_Title, beheld.Name, beheld.Female ? "Lady" : "Lord" );
										break;
									}
									else
									{
