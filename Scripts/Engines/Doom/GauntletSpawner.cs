		...
		public virtual int ComputeSpawnCount()
		{
			...
			if ( map != null )
			{
				...
				for ( int i = 0; playerCount == 0 && i < regions.Count; ++i )
				{
					...
					{
						for (  int j = 0;j < reg.Players.Count; j++ )
						{
							PlayerMobile pm = reg.Players[j] as PlayerMobile;
							if ( pm != null )
							{
								NetState state = pm.NetState;

								if ( state != null && !addresses.Contains( state.Address ) )
			...
			if ( addresses.Count == 0 && m_Region != null )
			{
				for (  int j = 0;j < m_Region.Players.Count; j++ )
				{
					PlayerMobile pm = m_Region.Players[j] as PlayerMobile;
					if ( pm != null )
					{
						NetState state = pm.NetState;
