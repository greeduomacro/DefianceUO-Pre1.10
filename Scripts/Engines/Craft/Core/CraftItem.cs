...
namespace Server.Engines.Craft
{
	...
	public class CraftItem
	{
		...
		// jakob, added this. you can now attach a method that is called to determine if you can make the item or not
		...
		public void Craft( Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool )
		{
			if ( from.BeginAction( typeof( CraftSystem ) ) )
			{
				...
				// jakob, added the thing that checks conditional. the SE stuff is there from before
				if( (!RequiresSE || (flags & 0x10) != 0) && (m_Conditional == null || m_Conditional( from )) )// SE 2D = 0x1F SE 3D = 0x11F
				// end
				{
		...
		public void CompleteCraft( int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool )
		{
			...
			if ( CheckSkills( from, typeRes, craftSystem, ref ignored, ref allRequiredSkills ) )
			{
				...
				if ( item != null )
				{
                    ...
                    //Al: Rewritten passage to properly stack created item
                    if (tool.Parent is Container)
                    {
                        ((Container)tool.Parent).TryDropItem(from, item, false);
                    }
                    else
                    {
