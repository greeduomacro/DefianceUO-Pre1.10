...
{
	public interface ISpawner //Will need to change later
	{
		void Spawn();
		void Respawn();
	...
	public class Spawner : Item, ISpawner
	{
        ...
        private ArrayList m_CreaturesProbability;
        ...
        public ArrayList CreaturesProbability
        {
...
//		[Constructable]
		...
		public void InitSpawn( int amount, TimeSpan minDelay, TimeSpan maxDelay, int team, int homeRange, ArrayList creaturesName )
		{
			...
			m_CreaturesProbability = new ArrayList();
		...
		public void Spawn()
		{
            if (m_CreaturesName.Count > 0)
            {
                ...
                //Calculate sum of all probabilities (equals 100%)
                int probabilitySum = 0;
                for (int i = 0; i < m_CreaturesProbability.Count; i++)
                    probabilitySum += (int)m_CreaturesProbability[i];

                //Determine creature to spawn
                for (int i = 0; i < m_CreaturesName.Count-1; i++)
                {
                    if (Utility.Random(probabilitySum) <= (int)m_CreaturesProbability[i])
                    {
                        Spawn(i);
                        return;
                    }
		...
		public override void Serialize( GenericWriter writer )
		{
            ...
            //Version 3
			writer.Write( m_CreaturesProbability.Count );

			for ( int i = 0; i < m_CreaturesProbability.Count; ++i )
				writer.Write( (int)m_CreaturesProbability[i] );
