...
// *** Added for Valor ***
		private ChampionIdol m_Idol;
		private bool m_IsValorUsed = false;
		private bool m_UseRestartDelay;

		public ChampionPlatform Platform
		{
			get { return m_Platform; }
			set { m_Platform = value; }
		}

		public ChampionAltar Altar
		{
			get { return m_Altar; }
			set { m_Altar = value; }
		}

		public ChampionIdol Idol
		{
			get { return m_Idol; }
			set { m_Idol = value; }
		}
		...
		public bool IsValorUsed
		{
			get { return m_IsValorUsed; }
			set { m_IsValorUsed = value; }
		}
// *** ***
...
// *** Added ***
		[CommandProperty( AccessLevel.GameMaster )]
		public bool UseRestartDelay
		{
			get
			{
				return m_UseRestartDelay;
			}
			set
			{
				if(m_UseRestartDelay = value)
				{
...
// *** Auto Cleaner
				...
				foreach( Item item in Map.GetItemsInRange( Location, m_SpawnRange ) )
					if( item is Corpse && CanBeDeleted((Corpse)item) )
						onGround.Add(item);
...
// ***
		private bool CanBeDeleted( Corpse corpse )
		{
			...
			{
				BaseCreature bc = (BaseCreature)corpse.Owner;
				if( bc != null && bc.IsChampionMonster )
					return true;
			}
			return false;
		}
			...
			if ( RandomizeType )
			{
				switch ( Utility.Random( 6 ) )
				{
					...
					case 1: Type = ChampionSpawnType.UnholyTerror; break;
                                        case 2: Type = ChampionSpawnType.ForestLord; break;
					case 3: Type = ChampionSpawnType.ColdBlood; break;
					case 4: Type = ChampionSpawnType.Abyss; break;
					case 5: Type = ChampionSpawnType.Arachnid; break;
				}
			}
...
// *** Added for Valor ***
					((BaseCreature)m).IsChampionMonster = true;
					((BaseCreature)m).SpawnLevel = (int) ( Level/4 );
// *** ***
				}
			}
		}
...
// *** Added for Valor ***
			writer.Write( m_IsValorUsed );
			writer.Write( m_Idol );
			writer.Write( m_UseRestartDelay );
// *** ***
