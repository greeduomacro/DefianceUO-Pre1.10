	...
	public abstract class Town : IComparable
	{
		...
		public void Capture( Faction f )
		{
			...
			// otherwise changing hands, income timer doesn't change
                        ...
                        // jakob, make sure to reset the deputycommander
			...
			ConstructGuardLists();
	                ...
	                // jakob, award online faction members players on capture
			foreach (NetState netState in NetState.Instances)
			{
				...
				if ( m != null )
				{
					...
					if ( pl != null && pl.Faction == f )
					{
						pl.KillPoints += 10;
						m.SendMessage( "You have been awarded 10 kill points because your faction captured {0}!", ToString());
					}
				}
			}
			// end
			...
			// jakob, bump all control points
				Monolith.CaptureControlPoints( f );
			// end
                }
