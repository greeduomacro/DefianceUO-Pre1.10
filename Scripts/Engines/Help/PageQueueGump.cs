	...
	public class PageEntryGump : Gump
	{
		...
		public PageEntryGump( Mobile m, PageEntry entry ) : base( 30, 30 )
		{
			try
			{
				...
				AddImageTiled( 0, 0, 410, 456+60, 0xA40 );
				AddAlphaRegion( 1, 1, 408, 454+60 );
				...
				AddImageTiled( 53, bottom - (buttons * 22) + 2, 338, 78+60, 0xBBC/*0x2426*/ );
				AddTextEntry( 55, bottom - (buttons++ * 22) + 2, 336, 78+60, 0x480, 0, "" );
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			switch ( info.ButtonID )
			{
				...
				case 4: // handle page
				{
					...
					{
						state.Mobile.SendMessage(0x85, "Someone is already handling that page." );
					}
				...
				case 5: // delete page
				{
					if ( m_Entry.Handler == null )
					{
                        PageSpyReport(m_Entry, state.Mobile, "[deleted]");
				...
				default:
				{
                    ...
                    if (index >= 0 && index < preresp.Count)
                    {
                        ...
                        PageSpyReport(m_Entry, state.Mobile, "[Predefined Response: " + ((PredefinedResponse)preresp[index]).Title + "]");
                    }
