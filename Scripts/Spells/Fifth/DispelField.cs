	...
	public class DispelFieldSpell : Spell
	{
		...
		public void Target( Item item )
		{
			...
			else if ( CheckSequence() )
			{
				...
				if(item is Moongate && ((Moongate)item).ExitGate != null)
				{
					Moongate exitGate = ((Moongate)item).ExitGate;
					Effects.SendLocationParticles( EffectItem.Create( exitGate.Location, exitGate.Map, EffectItem.DefaultDuration ), 0x376A, 9, 20, 5042 );
					Effects.PlaySound( exitGate.GetWorldLocation(), exitGate.Map, 0x201 );
					exitGate.Delete();
				}
