			...
			if ( from.AccessLevel > AccessLevel.Player || from.InRange( cont.GetWorldLocation(), 1 ) )
			{
				...
				if ( root != null && from.AccessLevel == AccessLevel.Player && from.Skills[SkillName.Snooping].Value < Utility.Random( 100 ) )
				{
					...
					if ( map != null )
					{
						...
						IPooledEnumerable eable = map.GetClientsInRange( from.Location, 3 );
				...
				{
					if ( from.AccessLevel == AccessLevel.Player && cont is TrapableContainer && ((TrapableContainer)cont).ExecuteTrap( from ) )
						return;
