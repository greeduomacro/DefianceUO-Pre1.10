			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( targeted is BaseCreature )
				{
					...
					if ( !c.IsDeadPet )
					{
						...
						{
							if ( !c.Controlled && from.Skills[SkillName.AnimalLore].Base < 100.0 )
								from.SendLocalizedMessage( 1049674 ); // At your skill level, you can only lore tamed creatures.
							else if ( (!c.Tamable && !c.Controlled) && from.Skills[SkillName.AnimalLore].Base < 110.0 )
	...
	public class AnimalLoreGump : Gump
	{
		...
		public AnimalLoreGump( BaseCreature c ) : base( 250, 50 )
		{
			...
			if ( Core.AOS )
			{
				...
				AddHtmlLocalized( 153, 294, 160, 18, (!c.Controlled || c.Loyalty == PetLoyalty.None) ? 1061643 : 1049594 + (int)c.Loyalty, LabelColor, false, false );
			}
			else
			{
				AddImage( 128, 278, 2086 );
			...
			if ( !Core.AOS )
			{
				...
				AddHtmlLocalized( 153, 240, 160, 18, (!c.Controlled || c.Loyalty == PetLoyalty.None) ? 1061643 : 1049594 + (int)c.Loyalty, LabelColor, false, false );
			}
