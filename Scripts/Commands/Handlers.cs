...
namespace Server.Scripts.Commands
{
	public class CommandHandlers
	{
		...
		private class ViewEqTarget : Target
		{
			...
			{
				ItemListEntry[] entries = new ItemListEntry[m.Items.Count + (m.Holding != null ? 1: 0)];
				int i;
				...
				{
					Item held = m.Holding;
					entries[i] = new ItemListEntry( String.Format( "{0}: {1}", Layer.Invalid, held.GetType().Name ), held.ItemID, held.Hue );
				}
		...
		private class BankTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				if ( targeted is Mobile )
				{
					...
					if ( box != null && ( from == m || from.AccessLevel > m.AccessLevel || from.AccessLevel == AccessLevel.Administrator ) )
					{
		...
		{
			BroadcastMessage( AccessLevel.Player, 0x35, String.Format( "Staff message from {0}:", e.Mobile.Name ) );
			BroadcastMessage( AccessLevel.Player, 0x35, e.ArgString );
		}
		...
		public static void BroadcastMessage( AccessLevel ac, int hue, string message )
		{
			foreach ( NetState state in NetState.Instances )
			{
				...
				{
					if ( ac >= AccessLevel.GameMaster && m.HueMod == 1010 )
						continue;
