...
namespace Server.Scripts.Commands
{
	...
	public class InterfaceItemGump : BaseGridGump
	{
		...
		public void Render()
		{
			...
			if (m_Item.RootParent is Mobile)
			{
				AddNewLine();
				AddEntryHtml( 160, ((Mobile)m_Item.RootParent).Name );
				AddEntryHeader( 20 );

				AddNewLine();
				AddEntryHtml( 20 + OffsetSize + 160, "Go to parent mobile" );
				AddEntryButton( 20, ArrowRightID1, ArrowRightID2, 7, ArrowRightWidth, ArrowRightHeight );

				AddNewLine();
				AddEntryHtml( 20 + OffsetSize + 160, "Bring parent mobile here" );
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
				...
				case 4: // Go there
				{
					InvokeCommand( String.Format( "Self Go {0}", m_Item.Serial.Value ) );
					break;
				}
				...
				case 6: // Bring to pack
				{
					...
					if ( owner != null && (owner.Map != null && owner.Map != Map.Internal) && owner.Hidden && owner.AccessLevel >= m_From.AccessLevel )
					{
				...
				case 7: // Go to parent mobile
				{
					if (!(m_Item.RootParent is Mobile))
						return;

					m_From.Location = ((Mobile)m_Item.RootParent).Location;
					break;


				}
				case 8: // Bring mobile parent to you
				{
					if (!(m_Item.RootParent is Mobile))
						return;

					Mobile parent = m_Item.RootParent as Mobile;
					parent.MoveToWorld( m_From.Location, m_From.Map);
					break;
				}
