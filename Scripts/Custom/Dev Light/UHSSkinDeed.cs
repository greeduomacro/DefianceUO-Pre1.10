...
{
	public class UHSSkinDeed : Item
	{
		[Constructable]
		public UHSSkinDeed() : base( 5360 )
		{
			Weight = 1.0;
		...
		public UHSSkinDeed( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			//if ( pm.Rank >= 7 ) // Change Min Rank To Use Here!!!
			{
				...
				{
					from.CloseGump( typeof( UHSSkinDeedGump ) );
					from.SendGump( new UHSSkinDeedGump( this ) );
				}
				else
				{
	...
	public class UHSSkinDeedGump : Gump
	{
		private UHSSkinDeed m_UHSSkinDeed;

		private class UHSSkinDeedEntry
		{
			...
			public UHSSkinDeedEntry( string name, int hueStart, int hueCount )
			{
				m_Name = name;
				m_HueStart = hueStart;
				m_HueCount = hueCount;
		...
		private static UHSSkinDeedEntry[] m_Entries = new UHSSkinDeedEntry[]
			{
				new UHSSkinDeedEntry( "*****", 1175, 1 ),
				//new UHSSkinDeedEntry( "*****", 1016, 15 ),
				//new UHSSkinDeedEntry( "*****", 1031, 15 ),
				//new UHSSkinDeedEntry( "*****", 1046, 11 ),
		};

		public UHSSkinDeedGump( UHSSkinDeed dye ) : base( 0, 0 )
		{
			m_UHSSkinDeed = dye;
			...
			for ( int i = 0; i < m_Entries.Length; ++i )
			{
				UHSSkinDeedEntry e = m_Entries[i];
		...
		{
			if ( m_UHSSkinDeed.Deleted )
				return;

			Mobile m = from.Mobile;
			int[] switches = info.Switches;

			if ( !m_UHSSkinDeed.IsChildOf( m.Backpack ) )
			{
			...
			if ( info.ButtonID != 0 && switches.Length > 0 )
			{
				...
				{
					UHSSkinDeedEntry e = m_Entries[entryIndex];
