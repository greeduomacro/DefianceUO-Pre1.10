...
{
	public class SnowEventPile : Item
	{
		...
		[Constructable]
		public SnowEventPile() : this( 25 )
		{
		}

		[Constructable]
		public SnowEventPile( int damage ) : base( 0x913 )
		{
			Hue = 0x480;
			Weight = 1.0;
			m_Damage = damage;
		...
		public SnowEventPile( Serial serial ) : base( serial )
		{
		}
		...
		private class InternalTimer : Timer
		{
			...
			{
				m_From.EndAction( typeof( SnowEventPile ) );
			}
		}
		...
		private class SnowTarget : Target
		{
			...
			private SnowEventPile m_Snow;

			public SnowTarget( Mobile thrower, SnowEventPile snow ) : base ( 10, false, TargetFlags.None )
			{
				m_Thrower = thrower;
				m_Snow = snow;
			}
			...
			protected override void OnTarget( Mobile from, object target )
			{
				...
				else if ( target is Mobile )
				{
					...
					if ( pack != null && pack.FindItemByType( new Type[]{ typeof( SnowEventPile ) } ) != null )
					{
						if ( from.BeginAction( typeof( SnowEventPile ) ) )
						{
							...
							AOS.Damage( targ, from, m_Snow.Damage, 100, 0, 0, 0, 0 );
						}
						else
						{
							from.SendLocalizedMessage( 1005574 ); // The snow is not ready to be packed yet.  Keep trying.
