...
namespace Server.Items
{
    public class SoulStone : Item
    {
        ...
        public virtual bool UseCharges { get { return false; } }
        public virtual bool ChangeItemIdWhenFilled { get { return true; } }
        ...
        public double SkillValue
        {
            ...
            set
            {
                ...
                // *** ***
                if (ChangeItemIdWhenFilled)
                {
                    if (m_SkillValue > 0.0)
                        this.ItemID = m_BaseItemID+1;
                    else
                        this.ItemID = m_BaseItemID;
                }
                // *** ***
        ...
        public SoulStone(string account)
            : this(account, Utility.RandomList( 0x2A93, 0x2ADC, 0x2AEC, 0x32F3 ) )
        {
        }
        ...
        public override void OnSingleClick(Mobile from)
        {
			...
			if (!this.IsEmpty)
            {
                LabelTo(from, 1070721, string.Format("#{0}\t{1:0.0}", 1044060 + (int)this.Skill, this.SkillValue)); // Skill stored: ~1_skillname~ ~2_skillamount~
            }

            if (this.UseCharges)
                LabelTo(from, 1060584, this.Charges.ToString()); // uses remaining: ~1_val~
        }
        // *** ***
        ...
        private bool CheckUse(Mobile from)
        {
            ...
            // *** ***
            else if (Charges <= 0 && this.UseCharges)
            {
