...
namespace Server.Items
{
	public class StoneFaceAddon : Item, IAddon, IChopable
	{
		...
		public override void OnMovement( Mobile m, Point3D oldLocation )
		{
			...
			{
				m_NextActiveTrigger = m_NextPassiveTrigger = DateTime.Now;

				OnTrigger( m );
			}
			else if ( CheckRange( m.Location, oldLocation, 2 ) && DateTime.Now >= m_NextPassiveTrigger )
			{
				m_NextPassiveTrigger = DateTime.Now;
