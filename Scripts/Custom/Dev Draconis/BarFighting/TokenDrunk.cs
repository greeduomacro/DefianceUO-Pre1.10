...
namespace Server.Mobiles
{
	public class TokenDrunk : BaseCreature
	{
		...
		public TokenDrunk() : base( AIType.AI_Melee, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
			...
			m_NextStealTime = DateTime.Now + TimeSpan.FromSeconds( Utility.RandomMinMax( 10, 15 ) );
			m_NextBabbleTime = DateTime.Now + TimeSpan.FromSeconds( Utility.RandomMinMax( 10, 40 ) );
		}
		...
		public override void OnThink()
		{
			...
			{
				m_NextStealTime = DateTime.Now + TimeSpan.FromSeconds( Utility.RandomMinMax( 5, 10 ) );
			...
			if ( DateTime.Now >= m_NextBabbleTime )
			{
				m_NextBabbleTime = DateTime.Now + TimeSpan.FromSeconds( Utility.RandomMinMax( 10, 40 ) );
