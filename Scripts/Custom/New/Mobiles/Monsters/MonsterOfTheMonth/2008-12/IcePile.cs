...
{
	public class IcePile : Item
	{
		[Constructable]
		public IcePile() : base( 0x913 )
		{
			Name = "super snow balls";
			Hue = 2066;
			Weight = 1.0;
		...
		public IcePile( Serial serial ) : base( serial )
		{
		}
		...
		private class InternalTimer : Timer
		{
			...
			public InternalTimer( Mobile from ) : base( TimeSpan.FromSeconds( 0.1 ) )
			{
				m_From = from;
			}
				...
				else if ( target is Mobile )
				{
					...
					if ( pack != null && pack.FindItemByType( new Type[]{ typeof( SnowPile ), typeof( PileOfGlacialSnow ), typeof( IcePile ) } ) != null )
					{
						if ( from.BeginAction( typeof( SnowPile ) ) )
						{
							...
							Effects.SendMovingEffect( from, targ, 0x36E4, 10, 0, false, true, 2066, 0 );
						}
						else
						{
							from.SendLocalizedMessage( 1005574 ); // The snow is not ready to be packed yet.  Keep trying.
