...
{
    public class TempDonationSkillBall : Item, ITempItem
	{
		...
		[Constructable]
		public TempDonationSkillBall( int SkillBonus, int days ) : base( 3699 )
		{
        ...
        public TempDonationSkillBall(Serial serial)
            : base(serial)
		{
		}
        ...
        {
            Name = String.Format("a limited skillball for {0} skill points, {1} days left to use this", m_SkillBonus, m_DaysLeft);
        }
...
{
	public class TempDonationSkillBallGump : Gump
	{
		private const int FieldsPerPage = 14;

		private Skill m_Skill;
		private TempDonationSkillBall m_skb;

		private static bool AllowedSkill( Skill skill )
		{
			return skill.SkillName != SkillName.AnimalTaming && skill.SkillName != SkillName.Provocation && skill.SkillName != SkillName.Blacksmith && skill.SkillName != SkillName.Tailoring && skill.SkillName != SkillName.RemoveTrap;
		}

		public TempDonationSkillBallGump ( Mobile from, TempDonationSkillBall skb ) : base ( 20, 30 )
		{
			m_skb = skb;
			...
			for ( int i = 0; i < ( skills.Length - number ); ++i )
			{
				...
				if ( (skill.Base + m_skb.SkillBonus) <= 100 && skill.Lock != SkillLock.Locked && skill.Lock != SkillLock.Down && AllowedSkill( skill ) )
				{
