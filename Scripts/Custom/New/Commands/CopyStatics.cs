...
namespace Arya.Misc
{
	...
	public class StaticsWorkshop
	{
		...
		{
			Server.Commands.Register( "CopyStatics", AccessLevel.GameMaster, new CommandEventHandler( OnCopyStatics ) );
		}
		...
		private static void OnCopyStatics( CommandEventArgs e )
		{
		...
		public int TilesCount
		{
			get
			{
				...
				foreach( ArrayList list in m_Tiles.Values )
				{
					count += list.Count;
				}
		...
		private class InternalTarget : Server.Targeting.Target
		{
			...
			public InternalTarget( StaticsWorkshop workshop, bool build ) : base( -1, build, Server.Targeting.TargetFlags.None )
			{
				m_Workshop = workshop;
				m_Build = build;
			}
		...
		private class InternalGump : Gump
		{
			...
			public InternalGump( StaticsWorkshop workshop ) : base( 50, 50 )
			{
				m_Workshop = workshop;
