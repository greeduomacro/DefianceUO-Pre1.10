...
namespace Server.Gumps
{
   public class FindSpawnsGump : Gump
   {
      ...
      {
         Commands.Register( "FindSpawns", AccessLevel.Seer, new CommandEventHandler( FindSpawns_OnCommand ) );
      }
      ...
      public FindSpawnsGump( Mobile owner, ArrayList list, int page ) : base( GumpOffsetX, GumpOffsetY )
      {
      ...
      {
         string mapName = null;
         if ( (args.Length > 0) && new ArrayList(Map.GetMapNames()).Contains(args[0]) )
         {
            mapName = args[0];
         }
         ...
         foreach (Item i in World.Items.Values)
         {
            ...
            {
               Spawner s = (Spawner)i;
               if ((mapName!=null) && !(s.Map.Name == mapName)) continue;

               names = s.CreaturesName;
            }
            else
            {
               continue;
            ...
            for (int index = (mapName!=null)? 1 : 0; index < args.Length; index++)
            {
               string criteria = args[index];
               meetsCriteria = false;

               foreach (string name in names)
               {
                  if ( name.ToUpper() == criteria.ToUpper() )
                  {
                     meetsCriteria = true;
                     break;
                  }
      ...
      public void Initialize( int page )
      {
         ...
         for ( int i = 0, index = page * EntryCount; i < EntryCount && index < m_Spawners.Count; ++i, ++index )
         {
            ...
            Item item = (Item)m_Spawners[index];
      ...
      private class DeleteGump : Gump
      {
         ...
         public override void OnResponse( Server.Network.NetState sender, RelayInfo info )
         {
            if ( info.ButtonID == 2 )
            {
               ((Item)m_Spawners[m_Index]).Delete();
