...
namespace Server.Gumps
{
	public class OnlineGump : Gump
	{
		...
		{
			//Commands.Register( "On", AccessLevel.Player, new CommandEventHandler( On_OnCommand ) );
		}
		...
		public OnlineGump( Mobile owner ) : this( owner, BuildList( owner ), 0 )
		{
		}

		public OnlineGump( Mobile owner, ArrayList list, int page ) : base( GumpOffsetX, GumpOffsetY )
		{
		...
		public void Initialize( int page )
		{
			...
			for ( int i = 0, index = page * EntryCount; i < EntryCount && index < m_Mobiles.Count; ++i, ++index )
			{
				...
				string name = m.Name;
				if ( m != null && m is PlayerMobile && (PlayerMobile)m != null && ((PlayerMobile)m).HasDonated )
					name = "* " + name;
