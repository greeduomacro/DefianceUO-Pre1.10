...
namespace Server.Items
{
	public class DDWayPoint : BaseAddon, IGameFlag
	{
		...
		public override void Deserialize( GenericReader reader )
		{
            ...
            if (m_DominationTime == 0) m_DominationTime = 10;
        ...
        public int DominationTime
        {
            get { return m_DominationTime; }
            set { if (value >= 1 && value < 100 && value % 10 == 0) m_DominationTime = value; }
        }
		...
		private class DominationTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				if ( m_Stage > 0 )
				{
					...
					if ( m_Stage <= m_Point.Components.Count*m_Divisor)
                        ((Item)m_Point.Components[(int)Math.Ceiling((double)m_Stage / m_Divisor) - 1]).Hue = m_Point.TeamOwner.Hue;

					if ( m_Stage <= m_Point.m_OtherPoint.Components.Count*m_Divisor)
                        ((Item)m_Point.m_OtherPoint.Components[(int)Math.Ceiling((double)m_Stage / m_Divisor) - 1]).Hue = m_Point.TeamOwner.Hue;
				}
				else
				{
